...
$quiz_lang = optional_param('quiz_lang', 'fr', PARAM_ALPHA);
...
if ($confirm && confirm_sesskey()) {
    ...
    if ($quiz_lang === 'en') {
        $quizdata->intro = '<p>Quiz automatically generated by AI with ' . count($ids) . ' questions.</p>';
    } else {
        $quizdata->intro = '<p>Quiz g&eacute;n&eacute;r&eacute; automatiquement par l\'IA avec ' . count($ids) . ' questions.</p>';
    }
    ...
    $msg = ($quiz_lang === 'en')
        ? "Quiz \"{$quizname}\" created with {$added} questions!"
        : "Quiz \"{$quizname}\" cree avec {$added} questions !";
    redirect($quizurl, $msg, null, \core\output\notification::NOTIFY_SUCCESS);
}
...
echo '<div class="form-group mt-3">';
echo '<label><strong>Langue du quiz :</strong></label>';
echo '<div class="form-check">';
echo '<input class="form-check-input" type="radio" name="quiz_lang" id="lang_fr" value="fr" checked>';
echo '<label class="form-check-label" for="lang_fr">Fran&ccedil;ais</label>';
echo '</div>';
echo '<div class="form-check">';
echo '<input class="form-check-input" type="radio" name="quiz_lang" id="lang_en" value="en">';
echo '<label class="form-check-label" for="lang_en">English</label>';
echo '</div>';
echo '</div>';
...
echo '<small class="form-text text-muted d-block">Quand l\'&eacute;tudiant se trompe, il verra l\'explication de l\'IA avec la bonne r&eacute;ponse. D&eacute;cochez pour un quiz sans feedback.</small>';
echo '</div>';
echo '</div>';
